...
class Process {
				...
				if (parameterFromCollection.getType().getClass()
					...
					this.opParametersReturn
							.put(parameterFromCollection.getName(),
									UML2mCRL2.determinePrimitiveType((PrimitiveTypeImpl) parameterFromCollection
											.getType()));
				}
	...
	public StringBuffer prepareForMCRL2() {
		StringBuffer buffer = new StringBuffer();
		if (classImpl == null && operationImpl == null)
			buffer.append("proc "+ this.getEnclosingInteraction().getName() + "(id:Nat) = \n");
		else {
...
//							+ sendEvent.getOperation().getName());
					if (occurence.getMessage().getMessageSort().toString()
						...
						theProcess.setEnclosingInteraction(el.getEnclosingInteraction());
						...
						if (!firstSendFound && insideOperand) {
							...
							if (operator.equals("alt")) {
								...
								loopProcess.setEnclosingInteraction(el.getEnclosingInteraction());
...
//							+ receiveEvent.getOperation().getName());
					...
					if (occurence.getMessage().getMessageSort().toString()
						...
						if (!findProcess.isProcessed) {
							findProcess.setEnclosingInteraction(el.getEnclosingInteraction());
						...
						// for synch_reply_receive we need the sum: ... parameters,
						// which
...
//					+ opaqueExpression.getBodies().get(0));
	...
	public static void createActionDefinitions() throws IOException {
		outfile.newLine();
//		System.out.println("\n\nact synch_call_send,synch_call_receive:Nat;");
	...
	public static void main(String args[]) throws Exception {
		if(args[0]==null || !args[0].endsWith(".uml")){
			System.out.println("Usage: UML2mCRL2 <exportedModel.uml>");
			System.exit(0);
		}
		int starter = 0;
...
//		System.out.println("rootPackage:" + rootPackage.getName());
		...
		createClassObjectSort(rootPackage);outfile.newLine();
		...
		outfile.newLine();
		outfile.write("%-------Process definitions--------");outfile.newLine();
...
//				System.out.println(process);
				if (process.getClassImpl() == null
					...
					starter++;
//					System.out.print("starter" + starter);
//					System.out.println("\t Enclosing interaction:"+process.getEnclosingInteraction().getName());
				} else {
...
//							+ process.getOpParametersReturn());
		...
		outfile.write("%-------End process definitions--------");outfile.newLine();
		outfile.newLine();
		outfile.write("%-------Init section--------");outfile.newLine();
		createInitSection();
//		System.out.println("SORTSTRING:" + SortString);
	...
	public static void createSortStrings() throws IOException {
		...
		outfile.write("%-------end sorts---------");outfile.newLine();
	}
	
	public static void createInitSection() throws IOException {
		outfile.newLine();
		outfile.write("init hide ({internal},");outfile.newLine();
		outfile.write("allow({internal,synch_call,synch_reply, asynch_call");outfile.newLine();
		outfile.write("},");outfile.newLine();
		outfile.write("comm({");outfile.newLine();
		outfile.write("synch_call_send|synch_call_receive->");
		outfile.write("synch_call,");outfile.newLine();
		outfile.write("synch_reply_receive|synch_reply_send->");outfile.newLine();
		outfile.write("synch_reply,");outfile.newLine();
		outfile.write("asynch_call_send|asynch_call_receive->");outfile.newLine();
		outfile.write("asynch_call");outfile.newLine();
		outfile.write("},");outfile.newLine();
		outfile.write("\t\t\t\t starter");outfile.newLine();
		outfile.write(")));");
